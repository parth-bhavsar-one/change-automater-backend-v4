function(properties, context) {
    ...
    let content = context.async( async ( callback ) => {
        ...
        let query = `SELECt * FROM spreadsheets_data WHERE id IN (SELECT dia_table_connection FROM detailed_impact_assessment WHERE id = ${properties.id})`;
        db.query(query)
        .then(async (data) => {

            let hli = await db.query(`SELECt * FROM spreadsheets_data WHERE id IN (SELECT hli_table_connection FROM high_level_impact_assesment WHERE id = ${properties.high_level_impact})`)
            ...
            hli = parseSpreadSheetData( (hli[0][0] || {}) )
            ...
            query += ` WHERE id IN ( SELECT dia_table_connection FROM detailed_impact_assessment WHERE id = ${properties.id} ) RETURNING *`
                ...
                if( ["impactedstakeholdergroup", "futureoperationalenvironment"].includes(d_config.orignal_title) ){
                    ...
                    hli.col_config.forEach((hli_config, hli_index) => {
                        if( d_config.src_col_title == hli_config.orignal_title ){
                            fieldsToUpdate1 += `, col_${hli_index+1} = ? `
            ...
            if( fieldsToUpdate1 && fieldsToUpdate1.length > 0){
                ...
                query1 += ` WHERE id IN ( SELECT hli_table_connection FROM high_level_impact_assesment WHERE id = ${properties.high_level_impact} ) RETURNING *`
